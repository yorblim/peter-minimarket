...
<div class="admin-container">
    ...
    <h1><i class="bi bi-tag"></i> Nueva Categoría</h1>

    <form action="{{ route('categorias.store') }}" method="POST" style="max-width:450px;">
        @csrf

        <div class="form-group">
            <label>Nombre de la categoría</label>
            <input type="text" name="nombre" value="{{ old('nombre') }}" required placeholder="Ej: Lácteos, Bebidas...">
            @error('nombre') <small style="color:#dc3545;">{{ $message }}</small> @enderror
        </div>

        <div>
            <button class="btn-admin-save"><i class="bi bi-check-lg"></i> Guardar</button>
